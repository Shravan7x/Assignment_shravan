...
> **🌐 Live Demo:** [https://assignment-shravan.vercel.app](https://assignment-shravan.vercel.app)
...
- **Frontend (Vercel)**: The React/Vite client is deployed to Vercel. SPA routing is managed seamlessly using `HashRouter` to prevent deep-link 404 errors across Vercel's Edge Network.
- **Backend (Render)**: The Node.js/Express API is deployed as a Web Service on Render, spinning up securely to handle API requests. Cross-Origin Resource Sharing (CORS) is enabled to accept requests exclusively from the Vercel frontend.
- **Database (Supabase)**: The PostgreSQL instance and S3-compliant blob storage for signatures are managed fully via Supabase's managed BaaS platform.
